...
class WoodcutterApp {
    ...
    displayCostInfo() {
        ...
        if (materialUsageRate) {
            materialUsageRate.textContent = `${costInfo.usageRate}%`;
        }
    }
    ...
    renderGroupCanvases() {
        ...
        uniqueBins.forEach((binInfo, index) => {
            ...
            header.innerHTML = `
                ...
                <span style="font-size: 13px;">효율: ${bin.efficiency.toFixed(1)}% | 절단: ${bin.cuttingCount}회</span>
            `;
    ...
    async addSummaryPageAsImage(doc) {
        ...
        y += 30;
        ctx.fillText(`재료 사용률: ${costInfo.usageRate}%`, margin + 20, y);
        y += 50;
        ...
        if (labeledGroups && labeledGroups.length > 0) {
            labeledGroups.forEach(group => {
                ctx.fillText(`${group.label}: ${group.width}×${group.height}mm - ${group.count}개`, margin + 20, y);
                y += 28;
            });
        }
    ...
    async addDiagramPageAsImage(doc, index) {
        ...
        // 정보
        ctx.font = '16px "Noto Sans KR", sans-serif';
        ctx.fillText(`효율: ${bin.efficiency.toFixed(1)}%  |  절단 횟수: ${bin.cuttingCount}회`, margin, y);
        y += 40;

        // 기존 캔버스 이미지 복사
        const scale = (tempCanvas.width - margin * 2) / canvas.width;
        const scaledHeight = canvas.height * scale;
        ctx.drawImage(canvas, margin, y, tempCanvas.width - margin * 2, scaledHeight);
